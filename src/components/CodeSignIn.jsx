...
import { verifyUserEnrolled } from '@/services/auth';
import { useRouter } from 'next/navigation';
import { Lock, CheckCircle, XCircle } from 'phosphor-react';
import toast from 'react-hot-toast';

export function CodeSignIn({ verificationId, resolver, currentUser }) {
    ...
    async function handleSubmit(e) {
        ...
        setIsLoading(true);
        try {
            const success = await verifyUserEnrolled({ verificationId, resolver }, code);
            if (success) {
                setIsValid(true);
                    ...
                    <div className="relative">
                        ...
                        <div className="relative flex justify-center">
                            <div className="bg-gray-900/90 p-2 rounded-full border border-cyan-500/30 shadow-lg shadow-cyan-500/20">
                                <Lock weight='fill' className='w-6 h-6 text-cyan-400'/>
                            </div>
                        </div>
                    </div>
